...
- 修复整页图片叠加文字或小图后绕过扁平化检测的问题；以实际可见覆盖范围提示人工复核，避免把大幅科研图直接判为错误。
- 增加按对象名称执行逻辑分组与原生连接线绑定的结构工具，用于编号、标签及连接图；处理后重新渲染和验证最终文件。
- 图片重建验收增加逐区域对照和代表性编辑操作，区分结构检查与真实应用中的联动行为。
- 配套增加扁平化负例和结构处理回归测试，并用 Blue Academic 四页样例复验；开发评测产物继续留在发布包之外。

## 3.5.0

- 新增图片底稿重建路径：支持从 PPT 截图、PNG/JPG、渲染 PDF 页、AI 视觉稿或网页截屏重建为可编辑 PPTX。
- 引入 SMART / PRESERVE / DEEP EDIT / SELECTIVE EDIT 的混合编辑颗粒度策略，优先恢复页面层原生对象，完整视觉资产按实际维护价值保留。
- 增加图片重建规则、生产路径说明、自然语言使用示例和图片来源下的对象/内容/科学证据边界。
- 明确禁止整页截图冒充可编辑、无意义碎片化和根据像素猜测数据；图片重建同样必须渲染、对比和运行最终 PPTX QA。

## 3.4.7

- 新增生产路径回显：新建整套 PPT、已批准样板扩展和大范围重设计开始前，先向用户说明范围、模板关系、制作路径、停点、交付物和关键假设。
- 对存在实质歧义或未明确授权长耗时执行的任务，回显后暂停等待确认；规划-only、局部修订和整篇机械修订不增加额外拦截。
- 保留 Route A/C 的 pilot 与样板审批，并将内部路线名、family/variant/archetype ID 留在内部。
- 增加运行时发布边界：开发文档、测试、评测协议和运行记录不进入 skill 安装包；发布脚本按白名单生成干净副本。
