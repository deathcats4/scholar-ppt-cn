...
Pilot、后续批次、失败重试和全稿逐页生图都必须重新注入同一段短约束，不能依赖长上下文记忆。文献汇报默认不使用“读图要点、关键认识、综合判断、支持证据、注意事项、证据观察、预期输出、本文切口”等栏目标签；“核心问题”和“结论”允许。生图模型可以设计来源支持的机制图，但不得自行增加科学节点、因果关系、数值或结论；推断、假说和跨文献综合需要明确标注并由用户确认。
